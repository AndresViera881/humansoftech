...
export default function Home() {
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: 'var(--bg)' }}>
      ...
      {/* ── Hero strip ── */}
      <div className="relative overflow-hidden" style={{ background: 'linear-gradient(135deg, #111827 0%, #1e3a5f 60%, #111827 100%)', borderBottom: '1px solid rgba(37,99,235,0.18)' }}>
        <div className="max-w-7xl mx-auto px-4 md:px-6 py-5 md:py-7 flex items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm mt-1" style={{ color: 'rgba(147,197,253,0.85)' }}>
          ...
          <div className="hidden sm:flex items-center gap-2 flex-shrink-0">
            {[
              ...
              <div key={label} className="flex items-center gap-1.5 px-3 py-2 rounded-xl"
                ...
                <svg className="w-3.5 h-3.5 flex-shrink-0" style={{ color: '#93c5fd' }} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>{icon}</svg>
            ...
            <div className="flex gap-2 overflow-x-auto pb-3 mb-2" style={{ scrollbarWidth: 'none' }}>
              {categoryNames.map(cat => (
                <button key={cat}
                  ...
                  style={{
                    background: selectedCategory === cat ? '#111827' : '#fff',
                    color: selectedCategory === cat ? '#fff' : '#374151',
                    border: `1.5px solid ${selectedCategory === cat ? '#111827' : 'rgba(0,0,0,0.1)'}`,
                    whiteSpace: 'nowrap',
                    boxShadow: selectedCategory === cat ? '0 2px 8px rgba(0,0,0,0.2)' : '0 1px 3px rgba(0,0,0,0.06)',
                  }}>
                  {cat}
                </button>
              ))}
