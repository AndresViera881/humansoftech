...
{/* Header */}
        <div className="relative px-6 pt-6 pb-4 text-center">
          ...
          <p className="text-xs mt-1" style={{ color: '#9ca3af' }}>
            Para finalizar tu pedido necesitas una cuenta
          </p>
        </div>

        {/* Tabs */}
        <div className="flex mx-6 mb-4 rounded-xl overflow-hidden" style={{ border: '1px solid #e5e7eb' }}>
          {(['login', 'register'] as const).map(t => (
            <button key={t} onClick={() => switchTab(t)}
              className="flex-1 py-2 text-xs font-bold transition-all"
              style={{ background: tab === t ? '#2563eb' : '#fff', color: tab === t ? '#fff' : '#6b7280' }}>
        ...
        <form onSubmit={handleSubmit} className="px-6 pb-6 flex flex-col gap-3">
          ...
          <input
            ...
            style={{ ...inputStyle, ...(focused === 'cedula' ? focusStyle : {}) }}
          />
          ...
          <div className="relative">
            <input
              ...
              style={{ ...inputStyle, paddingRight: '42px', ...(focused === 'password' ? focusStyle : {}) }}
            />
          ...
          <button
            ...
            style={{
              background: loading || !canSubmit ? 'rgba(37,99,235,0.1)' : '#2563eb',
              color: loading || !canSubmit ? 'rgba(37,99,235,0.4)' : '#fff',
