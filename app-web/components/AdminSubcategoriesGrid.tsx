    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <Input value={search} onChange={e => setSearch(e.target.value)} placeholder="Buscar subcategoría..." className="pl-9" />
        </div>
              ...
              {filtered.map((s, i) => (
                <div key={s.id} className="grid items-center px-4 py-3 transition-colors hover:bg-muted/20"
                  ...
                  <div className="flex items-center justify-center gap-1.5">
                    <Button variant="outline" size="sm" className="h-7 w-7 p-0" onClick={() => openEdit(s)}>
                    ...
                    <Button variant="ghost" size="sm" className="h-7 w-7 p-0 text-destructive hover:text-destructive hover:bg-destructive/10"
                      onClick={() => setDeleteConfirm(s)} disabled={deletingId === s.id}>
                      {deletingId === s.id
            ...
            <div className="flex sm:hidden flex-col divide-y">
              {filtered.map(s => (
                <div key={s.id} className="px-4 py-3 flex items-center gap-3 bg-white">
                  ...
                  <Button variant="outline" size="icon" className="h-8 w-8 flex-shrink-0" onClick={() => openEdit(s)}>
                    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                  </Button>
                  <Button variant="ghost" size="icon" className="h-8 w-8 flex-shrink-0 text-destructive hover:text-destructive hover:bg-destructive/10" onClick={() => setDeleteConfirm(s)}>
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex gap-3">
              ...
              <Button className="flex-1" onClick={handleSave} disabled={saving || !form.name.trim() || (!editTarget && !form.categoryId)}>
                {saving ? (
